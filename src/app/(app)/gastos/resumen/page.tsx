...
export default async function ResumenGastosPage() {
  const [casa, gastos, miembros] = await Promise.all([
    ...
    <main className="flex flex-1 justify-center bg-[radial-gradient(circle_at_top_left,_rgba(178,150,125,0.3),_transparent_34%)] bg-linen px-4 py-8">
      <section className="w-full max-w-2xl space-y-6">
        ...
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-cocoa">Resumen por mes</h1>
          <p className="mt-1 text-sm text-cocoa/70">Cuánto se gastó y quién pagó qué, mes por mes.</p>
        </div>

        {meses.length === 0 ? (
          <div className="rounded-lg border border-dashed border-camel bg-khaki/50 p-6 text-center shadow-sm">
            <p className="text-sm font-medium text-cocoa">Aún no hay gastos que resumir.</p>
          </div>
        ) : (
                    ...
                    .map(([usuarioId, monto]) => {
                      ...
                      return (
                        <li key={usuarioId} className="flex items-center justify-between gap-3 text-sm">
                          <span className="truncate text-cocoa">{miembro ? nombreMiembro(miembro) : 'Alguien'}</span>
