...
export default async function NuevoItemPage({
  ...
  const [referencia, { error }] = await Promise.all([getReferenciaCaducidad(), searchParams]);
    ...
    <main className="flex flex-1 justify-center bg-[linear-gradient(180deg,_#F5F1EA_0%,_#D7C9B8_100%)] px-4 py-8 dark:bg-none dark:bg-espresso">
      <div className="h-fit w-full max-w-sm space-y-6 rounded-lg border border-khaki bg-linen/95 p-6 shadow-lg dark:border-cocoa dark:bg-[#3a2820]">
        <div>
          ...
          <h1 className="mt-1 text-xl font-bold text-espresso dark:text-linen">Agregar producto</h1>
        </div>
        ...
        <ItemForm referencia={referencia} action={crearItem} textoBoton="Agregar al refri" />
      </div>
    </main>
  );
}
