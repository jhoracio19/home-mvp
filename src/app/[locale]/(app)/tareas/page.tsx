      ...
      <section className="mx-auto w-full max-w-2xl space-y-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-cocoa dark:text-linen">{t('titulo')}</h1>
          </div>
          ...
          <div className="flex items-center gap-4 rounded-lg border border-camel bg-khaki/60 px-4 py-3 dark:border-cocoa dark:bg-[#3a2820]/60">
            ...
            <div className="flex gap-6 text-sm">
              <div>
                ...
                <p className="text-xs text-cocoa/70">{t('tareaATiempo', { n: logros.puntos })}</p>
              </div>
              {logros.racha >= 2 && (
                <div>
                  ...
                  <p className="text-xs text-cocoa/70">{t('rachaSeguida')}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {completada && (
          <p className="flex items-center gap-2 rounded-lg border-2 border-camel bg-camel/25 px-4 py-3 text-sm font-semibold text-cocoa dark:text-linen">
            ...
            <span>
              {t('marcasteHecha', { nombre: completada })}
              {proxima && t('proximaVez', { proxima })}
            </span>
          </p>
        )}
        ...
        {(tareas.length > 0 || hayFiltros) && (
          <FormBusqueda>
            <div className="min-w-0 flex-1">
              <Input label={t('buscar')} name="q" defaultValue={q ?? ''} placeholder={t('nombreDeLaTareaPlaceholder')} />
            </div>
          </FormBusqueda>
        )}
        ...
        {tareasConUrgencia.length === 0 ? (
          <div className="rounded-lg border border-dashed border-camel bg-khaki/50 p-6 text-center shadow-sm dark:border-khaki/30 dark:bg-[#3a2820]/70">
            <p className="text-sm font-medium text-cocoa dark:text-khaki">
              {hayFiltros ? t('nadaCoincide') : t('sinTareas')}
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="mt-3 flex flex-wrap items-center gap-3">
                  <form action={completarTarea.bind(null, tarea.id)}>
                    <SubmitButton className="min-h-0 px-3 py-1.5 text-xs" pendingText={t('marcando')}>
                      {t('marcarHecho')}
                    </SubmitButton>
                  </form>
                  <Link
                  ...
                  <form action={eliminarTarea.bind(null, tarea.id)}>
                    <SubmitButton
                      ...
                      confirmMessage={t('confirmarEliminar', { nombre: tarea.nombre })}
                    >
                      {t('eliminar')}
                    </SubmitButton>
                  </form>
