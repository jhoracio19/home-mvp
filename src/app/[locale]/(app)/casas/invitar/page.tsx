...
export default async function InvitarPage({
  ...
  const [casa, rol, miembros, siteUrl, usuario, { error }, t, locale] = await Promise.all([
    getCasaActivaOrRedirect(),
    getRolEnCasaActiva(),
    getMiembrosCasaActiva(),
    getSiteUrl(),
  ...
  const codigoVigente =
    casa.codigo_invitacion && casa.codigo_invitacion_expira && new Date(casa.codigo_invitacion_expira) > new Date()
      ...
      : null;
  const linkInvitacion = codigoVigente ? `${siteUrl}/${locale}/casas/unirse?codigo=${codigoVigente}` : '';
    ...
    <main className="flex flex-1 justify-center bg-[radial-gradient(circle_at_top_left,_rgba(178,150,125,0.3),_transparent_34%)] bg-linen px-4 py-8 dark:bg-none dark:bg-espresso">
      <div className="h-fit w-full max-w-sm space-y-6">
        ...
        <div>
          ...
          <h1 className="mt-1 text-xl font-bold text-cocoa dark:text-linen">{t('invitarMiembros')}</h1>
        </div>
        ...
        <div className="space-y-4 rounded-lg border border-camel bg-khaki p-6 shadow-lg dark:border-cocoa dark:bg-[#3a2820]">
          {!esAdmin ? (
            <p className="text-sm font-medium text-cocoa dark:text-khaki">{t('soloAdminInvitar')}</p>
          ) : codigoVigente ? (
            <>
              <div>
                ...
                <p className="mt-3 text-xs text-cocoa dark:text-khaki">
                  {t('validoHasta', {
                    fecha: new Date(casa.codigo_invitacion_expira!).toLocaleDateString(intlLocale(locale)),
                  })}
                </p>
              ...
              <form action={generarCodigoInvitacion}>
                <SubmitButton variant="secondary" className="w-full" pendingText={t('generando')}>
                  {t('generarCodigoNuevo')}
                </SubmitButton>
              </form>
            </>
          ) : (
            <>
              <p className="text-sm font-medium text-cocoa dark:text-khaki">{t('sinCodigoActivo')}</p>
              <form action={generarCodigoInvitacion}>
                <SubmitButton className="w-full" pendingText={t('generando')}>
                  {t('generarCodigoInvitacion')}
                </SubmitButton>
              </form>
            </>
          )}
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-bold text-cocoa dark:text-linen">
            {t('miembrosActuales', { n: miembros.length })}
          </h2>
                ...
                >
                  <div className="min-w-0">
                    <p className="truncate font-medium text-cocoa dark:text-linen">
                      {nombreMiembro(m)}
                      {esUnoMismo && <span className="text-cocoa/60">{t('tu')}</span>}
                    </p>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <span className="text-xs font-semibold uppercase tracking-wide text-cocoa dark:text-camel">
                      {m.rol === 'admin' ? t('rolAdmin') : t('rolMiembro')}
                    </span>
                    {esAdmin && !esUnoMismo && (
                      <form action={quitarMiembro.bind(null, m.usuario_id)}>
                        <SubmitButton
                          ...
                          pendingText={t('quitando')}
                          confirmMessage={t('confirmarQuitarMiembro', { nombre: nombreMiembro(m) })}
                        >
                          {t('quitar')}
                        </SubmitButton>
                      </form>
